...
const storageMulter = multer.memoryStorage();  // Define storageMulter
  ...
  try {
    const fileUrls = await Promise.all(
      Object.keys(req.files).map(async (fieldName) => {
        const file = req.files[fieldName][0]; // Only one file per field is allowed
        ...
        return new Promise((resolve, reject) => {
          ...
          blobStream.on('finish', () => {
            resolve({ fieldName, url: `https://console.cloud.google.com/storage/browser/${bucket.name}/${uniqueFileName}` });
          });
